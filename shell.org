...
#+begin_src bash
  ...
  # The custom npm global directory, enables user-specific "global" package
  # installations on Linux without requiring `sudo`. MacOS does not need this
  # configuration since npm global installations use Homebrew's bin directory.
...
#+begin_src bash
  ...
  MY_YELLOW='\x01\e[33m\x02'
  MY_LYELLOW='\x01\e[93m\x02'

  PS1='\n'                        # An extra line to separate previous output and PS1
  ...
  PS1=$PS1'$(printf " ${MY_BLUE}\u@\h")'                # user@host (Updated variable name)
  PS1=$PS1'$(printf ":${MY_LBLUE}\w")'                  # working directory
  PS1=$PS1'\n'                                          # end of second line
...
Uncomment and adjust this block if multiple config files are used.
...
#+begin_src bash
  # The token was stored in MacOS keychain using:
  #  security add-generic-password -a "jira-api-token" -s "jira-cli" -w "YOUR_TOKEN"
  export JIRA_API_TOKEN=$(security find-generic-password -a "jira-api-token" -s "jira-cli" -w 2>/dev/null)
#+end_src
...
#+begin_src fish :tangle ~/.config/fish/config.fish :mkdirp yes
  # On macOS, Pyright (run by Emacs eglot) can easily exceed the default value in
  # non-trivial Python projects.
  if test (ulimit -n) -lt 65536
      ulimit -n 65536
  end
  ...
  # dvisvgm (from MacTeX) can't find Ghostscript's libgs on Apple Silicon
  # where Homebrew installs to /opt/homebrew/lib instead of /usr/local/lib.
  if test -e /opt/homebrew/lib/libgs.dylib
      set -gx LIBGS /opt/homebrew/lib/libgs.dylib
  end

  # KUBECONFIG is not set/exported: use the default ~/.kube/config. Export
  # KUBECONFIG only when override is needed.

  # JIRA API TOKEN
  if test "$TERM" != dumb        # Don't load this TOKEN for Emacs Tramp etc.
      # pick whichever secret store exists on this host
      if type -q security          # macOS Keychain
          set -gx JIRA_API_TOKEN (security find-generic-password -a "$USER" -s jira-cli -w 2>/dev/null)
      else if type -q pass         # Linux: GPG-backed pass
          set -gx JIRA_API_TOKEN (pass show jira-cli 2>/dev/null)
      end
  end
  ...
  # installations on Linux without requiring `sudo`. MacOS does not need this
  # configuration since npm global installations use Homebrew's bin directory.
  # IMPORTANT: must run `npm config set prefix "$NPM_GLOBAL"` once.
  set -gx NPM_GLOBAL $HOME/.npm-global

  fish_add_path -g ~/.local/bin
  fish_add_path -g $NPM_GLOBAL/bin
  fish_add_path -g ~/.opencode/bin # On Ubuntu, opencode is installed here
#+end_src
...
function fish_prompt
    # Save the status of the last run command immediately
    set -l last_status $status

    # Define Colors
    set -l MY_RESET (set_color normal)
    set -l MY_REVERSE (set_color -r)

    set -l MY_GREEN (set_color green)
    set -l MY_BLUE (set_color blue)
    set -l MY_PURPLE (set_color magenta)
    set -l MY_GRAY (set_color brwhite)

    set -l MY_DGRAY (set_color brblack)
    set -l MY_LGREEN (set_color brgreen)
    set -l MY_LBLUE (set_color brblue)

    set -l MY_YELLOW (set_color yellow)
    set -l MY_LYELLOW (set_color bryellow)

    # 1. Print an extra line to separate previous output
    echo ""

    # 2. Start First Line
    echo -n "$MY_GREEN╭"

    # Helper: Python Virtual Env
    if set -q VIRTUAL_ENV
        # Replaces $HOME with ~ just like your Bash variant
        set -l venv_path (string replace -r "^$HOME" "~" $VIRTUAL_ENV)
        echo -n " $MY_PURPLE $MY_DGRAY$venv_path"
    end
    ...
    if type -q git
        ...
        if test -n "$branch"
            echo -n " $MY_GRAY $MY_DGRAY$branch"
        end
    end

    # Helper: Kubernetes Context
    if type -q kubectl
        ...
        if test -n "$kube_context"; or test -n "$kube_namespace"
            echo -n " $MY_GRAY󰠳 $MY_DGRAY$kube_context/$kube_namespace"
        end
    end

    # Helper: Command Number & Time
    # Note: Fish tracks history indices slightly differently than Bash
    set -l cmd_num (history | count)
    set -l current_time (date +"%T%z")
    echo -n " $MY_GRAY󰞷 $MY_DGRAY$cmd_num"
    echo -n " $MY_GRAY $MY_DGRAY$current_time"
    ...
    if set -q MY_WARN; and test -n "$MY_WARN"
        echo -n " $MY_REVERSE$MY_PURPLE$MY_WARN$MY_RESET"
    end

    # User, Host, and PWD
    # Fish shortens PWD automatically if desired, prompt_pwd can be used instead of $PWD
    set -l pwd_display (string replace -r "^$HOME" "~" $PWD)
    echo -n " $MY_BLUE$USER@"(prompt_hostname)
    echo -n ":$MY_LBLUE$pwd_display"

    # End Second Line
    echo ""

    # 4. Third Line (Prompt Symbol)
    # Automatically switch between '#' for root and '$' for standard users
    set -l symbol "\$"
    if functions -q fish_is_root_user; and fish_is_root_user
        set symbol "#"
    end

    echo -n "$MY_GREEN╰ $symbol $MY_RESET"
end
#+end_src
...
#+begin_src fish :tangle ~/.config/fish/functions/ep.fish :mkdirp yes
function ep --description 'go to current directory of Emacs daemon'
    # cd to the working directory of the Emacs daemon's current buffer.
    # Usage: ep [-s SESSION]
    argparse "s/session"= -- $argv
    or return
    set -l args
    if set -q _flag_session
        set args -s "$_flag_session"
    end
    cd (emacsclient $args -e '(expand-file-name
        (with-current-buffer (window-buffer) default-directory))' | string trim -c '"')
end
#+end_src

*** et

#+begin_src fish :tangle ~/.config/fish/functions/et.fish :mkdirp yes
  function et --wraps='emacsclient -a "" -t' --description 'emacsclient in terminal with optional named session'
      # Connect to (or auto-start) an Emacs daemon and open FILEs in a terminal frame.
      # Usage: et [-s SESSION] [FILE...]
      argparse "s/session"= -- $argv
      or return
      set -l files $argv
      if set -q _flag_session
          emacsclient -t -s "$_flag_session" $files 2>/dev/null
          or begin; emacs --daemon="$_flag_session"; and emacsclient -t -s "$_flag_session" $files; end
      else
...
#+begin_src fish :tangle ~/.config/fish/functions/git_clean_branches.fish :mkdirp yes
  function git_clean_branches
      argparse 'n/not-dry-run' 'm/master=' -- $argv
      or return 1

      git fetch -p

      set -l master_branch_name
      if set -q _flag_master
          set master_branch_name $_flag_master
      else
          # Pick a remote: prefer origin, else the sole remote
          ...
          # Ask the remote what its default branch is (e.g. origin/main -> main)
          if test -n "$remote"
              set -l remote_head (git symbolic-ref --quiet --short refs/remotes/$remote/HEAD)
              if test -n "$remote_head"
                  set master_branch_name (string replace "$remote/" '' $remote_head)
              end
          end

          # Fall back to common local branch names
          if test -z "$master_branch_name"
              for candidate in main master
                  if git show-ref --verify --quiet refs/heads/$candidate
                      set master_branch_name $candidate
                      break
                  end
              end
          end
      ...
      if test -z "$master_branch_name"
          echo "error: could not detect the default branch; specify it with -m <branch>" >&2
          return 1
      end

      if not git show-ref --verify --quiet refs/heads/$master_branch_name
          echo "error: branch '$master_branch_name' does not exist in this repo" >&2
          return 1
      end

      set -l merged (git for-each-ref \
          --format='%(refname:short)' \
          --merged=refs/heads/$master_branch_name \
          --exclude=refs/heads/$master_branch_name \
          refs/heads)

      set -l remote_deleted (git for-each-ref \
      ...
      if set -q _flag_not_dry_run
          for each in $merged $remote_deleted
              git branch -D $each
          end
      else
...
#+begin_src fish :tangle ~/.config/fish/functions/tg.fish :mkdirp yes
  function tg --description "Attach to specified tmux session"
      # 1. If the session does not exist, create it.
      # 2. If no session name specified, prompt to choose from existing ones.
      ...
      while true
          read -P "Select a session: " reply

          if string match -qr '^[1-9][0-9]*$' -- $reply
              and test $reply -le (count $sessions)
              tmux -2 attach -t $sessions[$reply]
              return
          end
...
Convenience wrappers to stop/start my Evergreen workstation (spawn host
=i-00a993ea98ed9536b=). =ws-start= waits until the host is running so it is
ready to SSH into when the command returns; =ws-stop= returns immediately
(the Friday-night uptime schedule acts as the safety net). Extra arguments,
if any, are forwarded to the underlying evergreen command.

#+begin_src fish :tangle ~/.config/fish/functions/ws-stop.fish :mkdirp yes
  function ws-stop --description 'Stop my Evergreen workstation (returns immediately)'
      evergreen host stop --host i-00a993ea98ed9536b $argv
  end
#+end_src

#+begin_src fish :tangle ~/.config/fish/functions/ws-start.fish :mkdirp yes
  function ws-start --description 'Start my Evergreen workstation and wait until ready'
      evergreen host start --host i-00a993ea98ed9536b -w $argv
  end
#+end_src

*** ec2s

#+begin_src fish :tangle ~/.config/fish/functions/ec2s.fish :mkdirp yes
  function ec2s
      # List EC2 instances from a Terraform state file, grouped by module. For
      # each aws_instance resource, prints the module name followed by a
      # tab-separated row per instance: Name tag, public IP, private IP, AZ.
      # Defaults to ./terraform.tfstate; pass a path to use a different file.

      set -l tf_state_file "./terraform.tfstate"

      if test (count $argv) -gt 1
          echo "Usage:" >&2
          echo "ec2s [path/to/the/tfstate_file]" >&2
          echo "" >&2
          return 1
      end

      if test (count $argv) -eq 1
          set tf_state_file $argv[1]
      end

      jq -r '
      .resources[]
      | select(.type=="aws_instance" and .instances!=[])
      | (.module|sub("^.*\\\\."; "")) + ":",
      (.instances[].attributes| ["", .tags.Name, .public_ip, .private_ip, .availability_zone]|@tsv),
      ""
      ' <$tf_state_file
  end
#+end_src
