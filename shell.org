...
#+begin_src bash
  ...
  # Add the user's custom npm global directory to the PATH. This is a common
  # practice on Linux to allow global package installation without requiring
  # `sudo`. On macOS, this is typically handled by Homebrew, so this step is not
  # required.
