...
#+begin_src fish :tangle ~/.config/fish/config.fish
  if status is-interactive
      if test "$TERM" = xterm-256color; and not set -q TMUX
          tg
      end
  end
  ...
  # JIRA API TOKEN
  function jira --wraps jira --description 'load JIRA token on first use, then run jira'
      # Lazy-load avoids Emacs remote access hang on password prompt
...
function fish_prompt
    ...
    if type -q kubectl
        ...
        if test -n "$kube_context"; or test -n "$kube_namespace"
            echo -n " $MY_GRAY   $MY_DGRAY$kube_context/$kube_namespace"
        end
    end
    ...
    echo -n " $MY_GRAY   $MY_DGRAY$cmd_num"
...
#+begin_src fish :tangle ~/.config/fish/functions/ep.fish
...
#+begin_src fish :tangle ~/.config/fish/functions/git_clean_branches.fish
  function git_clean_branches
      argparse 'n/not-dry-run' 'm/master=' -- $argv
      or return 1

      set -l master_branch_name master
      set -q _flag_master; and set master_branch_name $_flag_master
...
#+begin_src fish :tangle ~/.config/fish/functions/tg.fish
...
#+begin_src fish :tangle ~/.config/fish/functions/ec2s.fish
  function ec2s
