...
#+begin_src toml :tangle ~/.config/aerospace/aerospace.toml :mkdirp yes
  ...
  [mode.jump.binding]
  j = ['exec-and-forget ~/.config/aerospace/aerospace-jump-app.sh WezTerm', 'mode main']
  k = ['exec-and-forget ~/.config/aerospace/aerospace-jump-app.sh "Google Chrome"', 'mode main']
  l = ['exec-and-forget ~/.config/aerospace/aerospace-jump-app.sh Slack', 'mode main']
  i = ['exec-and-forget ~/.config/aerospace/aerospace-jump-app.sh Emacs', 'mode main']
  o = ['exec-and-forget ~/.config/aerospace/aerospace-jump-app.sh Safari', 'mode main']
  esc = 'mode main'
  ctrl-g = 'mode main'

  [mode.summon.binding]
  j = ['exec-and-forget ~/.config/aerospace/aerospace-summon-app.sh WezTerm', 'mode main']
  k = ['exec-and-forget ~/.config/aerospace/aerospace-summon-app.sh "Google Chrome"', 'mode main']
  l = ['exec-and-forget ~/.config/aerospace/aerospace-summon-app.sh Slack', 'mode main']
  i = ['exec-and-forget ~/.config/aerospace/aerospace-summon-app.sh Emacs', 'mode main']
  o = ['exec-and-forget ~/.config/aerospace/aerospace-summon-app.sh Safari', 'mode main']
  esc = 'mode main'
  ctrl-g = 'mode main'
...
#+begin_src bash :tangle ~/.config/aerospace/aerospace-swap-workspaces.sh :shebang "#!/usr/bin/env bash"
  ...
  aerospace workspace "$src_ws"
#+end_src
** Script to summon an application to the current workspace

#+begin_src bash :tangle ~/.config/aerospace/aerospace-summon-app.sh :shebang "#!/usr/bin/env bash"
# Summon an application:
#   - If it is running, move all of its windows to the focused workspace
#     and focus the first one.
#   - If it is not running, launch it (it will appear on the focused
#     workspace).
# Usage: aerospace-summon-app.sh <app-name>
# Example: aerospace-summon-app.sh WezTerm
...
# Collect window IDs whose app-name matches (case-insensitive).
...
WINDOW_IDS=$(aerospace list-windows --all --format "%{window-id}|%{app-name}" \
    | awk -F'|' -v app="$APP_NAME" 'tolower($2) == tolower(app) {print $1}')
...
[ -n "$FIRST_WID" ] && aerospace focus --window-id "$FIRST_WID"
#+end_src

** Script to jump to an application

#+begin_src bash :tangle ~/.config/aerospace/aerospace-jump-app.sh :shebang "#!/usr/bin/env bash"
  # Jump to an application:
  #   - If it is running, focus its (first) window wherever it lives,
  #     switching to that window's workspace as needed.
  #   - If it is not running, launch it.
  # Usage: aerospace-jump-app.sh <app-name>
  # Example: aerospace-jump-app.sh WezTerm
  ...
  # First window ID whose app-name matches (case-insensitive).
  # The "|" delimiter avoids issues with app names containing spaces.
  FIRST_WID=$(aerospace list-windows --all --format "%{window-id}|%{app-name}" \
      | awk -F'|' -v app="$APP_NAME" 'tolower($2) == tolower(app) {print $1; exit}')
