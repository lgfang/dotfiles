# -*-mode:org; coding:utf-8; time-stamp-pattern:"8/#\\+DATE:[ \t]+%Y-%02m-%02d %5z$" -*-
# Created: Fang Lungang 2024-08-17
#+export_exclude_tags: noexport

#+TITLE: My Terminal Emulator Configurations
#+AUTHOR: Fang Lungang
#+DATE: 2026-08-22 +1000

* Timestamp                                                        :noexport:

#+name: tangle-time-stamp
#+begin_src emacs-lisp :tangle no
(concat user-full-name " " (format-time-string "%Y-%m-%d %5z"))
#+end_src

#+begin_src lua :tangle ~/.wezterm.lua :noweb yes
  -- Created: Fang Lungang 2024-08-17
  -- Updated: <<tangle-time-stamp()>>
#+end_src

* WezTerm

** Why WezTerm over iTerm2

As a Tmux + Emacs user on macOS, I don't need a built-in multiplexer, tabs, or
panes. That narrows the choice down to a few points that actually matter to me:
- Configured via a file rather than a GUI, which means it is
  - version-controllable, and
  - faster to change settings - editing a config file beats hunting through a
    maze of nested preference tabs and panes.
- Custom hyperlink rules.

** Config Initialization
#+begin_src lua :tangle ~/.wezterm.lua
  local wezterm = require 'wezterm'
  local config = wezterm.config_builder()
#+end_src

** Appearance

*** Window and frame

#+begin_src lua :tangle ~/.wezterm.lua
  config.window_decorations = 'RESIZE'
  config.window_padding = { left = 0, right = 0, top = 0, bottom = 0}
  config.enable_tab_bar = false
#+end_src

*** Fonts

**** Font

I no longer use custom fonts, so neither block below is tangled. To use custom
fonts, tangle them (remove ~:tangle no~) and tweak to taste. See [[https://github.com/ryanoasis/nerd-fonts/discussions/1103][this post]] for
the differences between the 3 nerd font variants (nerd font, nerd font mono, and
nerd font propo).

~config.font~ takes a single font object. The ~font_with_weight~ helper builds
one, so the same family/fallback chain can be reused at different weights.

On fallback fonts:
- ~wezterm.font_with_fallback~ takes a list: the first entry is the main font,
  the rest are consulted in order for glyphs the main font lacks.
- WezTerm ships built-in fallbacks, so setting them is optional. Normally it is
  done to deliberately mix fonts.
- ~STKaiti~ is added to display Chinese characters (otherwise WezTerm uses the
  system default). It must be placed *last* so ASCII/Latin glyphs do not fall
  back to it, since a CJK font also covers punctuation and full-width Latin,
  which is usually undesirable.
- The other entries mirror WezTerm's own defaults, kept here as the place to
  edit for further overrides.

#+begin_src lua :tangle ~/.wezterm.lua :tangle no
  -- Liga SFMono Nerd Font: brew install --cask font-sf-mono-nerd-font-ligaturized
  -- FiraCode Nerd Font: brew install --cask font-fira-code-nerd-font
  local font_family = 'Liga SFMono Nerd Font'

  local fallback_fonts = {
      'JetBrains Mono',
      'Noto Color Emoji',
      'Symbols Nerd Font Mono',
      'STKaiti',
  }

  -- Helper function to create a font object with fallback
  local function font_with_weight(weight)
      return wezterm.font_with_fallback({
              { family = font_family, weight = weight },
              table.unpack(fallback_fonts)
      })
  end

  config.font = font_with_weight('Regular')
#+end_src

Optionally set weights per intensity. An unavailable weight silently falls back
to the closest one.
#+begin_src lua :tangle ~/.wezterm.lua :tangle no
  local font_rules = {
      {
          -- NOTE: this intensity rule shadows config.font for ordinary text
          intensity = 'Normal',
          font = font_with_weight('Medium'),
      },
      {
          -- Increase the weight of Bold text
          intensity = 'Bold',
          font = font_with_weight('ExtraBold'),
      },
  }

  config.font_rules = font_rules
#+end_src

**** Size and ligatures

#+begin_src lua :tangle ~/.wezterm.lua
  config.font_size = 16

  -- Ligature settings. The selected font must supports theses features. (Nerd
  -- Fonts does)
  config.harfbuzz_features = {
     -- Contextual Alternates, '->', '!=' etc.
     'calt=1',
     -- Contextual Ligatures
     'clig=0',
     -- Standard Ligatures
     'liga=0',
  }
#+end_src

*** Brightness and colour

#+begin_src lua :tangle ~/.wezterm.lua
  config.foreground_text_hsb = {
     hue = 1.0,
     saturation = 1.0,
     brightness = 0.7,
  }

  config.bold_brightens_ansi_colors = "No"
#+end_src

NOTE: to test ~bold_brightens_ansi_colors~, verify the results in a shell CLI.
Do NOT do it within Emacs, as Emacs themes may apply their own brightness
adjustments that could interfere with the results.

Use the following commands to display bold and normal text with colours:
#+begin_src bash
  echo -e "\e[1;31mBold Red\e[0m vs \e[31mNormal Red\e[0m"
  echo -e "\e[1;32mBold Green\e[0m vs \e[32mNormal Green\e[0m"
  echo -e "\e[1;34mBold Blue\e[0m vs \e[34mNormal Blue\e[0m"
  echo -e "\e[1;33mBold Yellow\e[0m vs \e[33mNormal Yellow\e[0m"
#+end_src

*** COMMENT Opacity

#+begin_src lua :tangle ~/.wezterm.lua
  config.window_background_opacity = 0.8
#+end_src
This is interesting, but I often prefer to disable it as the background can be
distracting.

Additionally, for Emacs running in the terminal to support opacity, set its
background to "unspecified-bg" as shown below:
#+begin_src emacs-lisp
  (set-face-background 'default "unspecified-bg")
#+end_src

*** Color theme

The remote window uses a different scheme, keyed off the tmux window
title prefix set by =set-titles-string= (see tmux.org).

#+begin_src lua :tangle ~/.wezterm.lua
  config.color_scheme = 'GruvboxDark'

  wezterm.on(
      'update-right-status',
      function(window, pane)
          local title = pane:get_title()
          if title and title:match('^ws:') then
              window:set_config_overrides({ color_scheme = 'Solarized Dark - Patched' })
          else
              window:set_config_overrides({ color_scheme = 'GruvboxDark' })
          end
  end)
#+end_src

** Bell

Terminal should ring a bell (alert) upon the ascii character BEL (~printf '\a'~).
#+begin_src lua :tangle ~/.wezterm.lua
  config.audible_bell = "SystemBeep"
  config.visual_bell = {
     fade_in_duration_ms = 75,
     fade_out_duration_ms = 75,
     target = "BackgroundColor"
  }
  config.colors = {
     visual_bell = '#202020',
  }
#+end_src

NOTE: This does not work in the Emacs TUI, as its constant redrawing obscures
the terminal emulator's visual effects. Use ~ring-bell-function~ in Emacs for a
custom effect like flashing the mode line.

** Keyboard
#+begin_src lua :tangle ~/.wezterm.lua
  -- Do not set left/right "Alt as "AltGr"
  config.send_composed_key_when_left_alt_is_pressed = false
  config.send_composed_key_when_right_alt_is_pressed = false

  config.disable_default_key_bindings = true
  config.native_macos_fullscreen_mode = true

  config.keys = {
     { action = wezterm.action.CloseCurrentTab {confirm = true}, key = 'w', mods = 'CMD'},
     { action = wezterm.action.QuitApplication, key = 'q', mods = 'CMD'},
     { action = wezterm.action.Hide, key = 'h', mods = 'CMD'},
     { action = wezterm.action.SpawnWindow, key = 'n', mods = 'CMD'},

     { action = wezterm.action.DecreaseFontSize, key = '-', mods = 'CMD'},
     { action = wezterm.action.IncreaseFontSize, key = '=', mods = 'CMD'},

     { action = wezterm.action.PasteFrom 'Clipboard' , key = 'v', mods = 'CMD', },
     { action = wezterm.action.PasteFrom 'PrimarySelection', key = 'v', mods = 'CMD', },
     { action = wezterm.action.ToggleFullScreen, key = 'f', mods = 'CMD|CTRL',},
  }
#+end_src

** Custom hyperlinks
#+begin_src lua :tangle ~/.wezterm.lua
  -- Use the defaults as a base
  config.hyperlink_rules = wezterm.default_hyperlink_rules()
  table.insert(
     config.hyperlink_rules,
     {
        -- example: clicking on SERVER-1234 opens this mongodb sever ticket
        regex = [[\b((SERVER|server)-[0-9]+)\b]],
        format = 'https://jira.mongodb.org/browse/$1'
     }
  )
#+end_src

** Return the config object (end of the config)
#+begin_src lua :tangle ~/.wezterm.lua
  return config
#+end_src
