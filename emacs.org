...
#+begin_src emacs-lisp
  ...
  also allows entering any path with directory-name autocompletion."
    ...
    (defun my/agent-shell-new-worktree-shell ()
      "A wrapper of `agent-shell-new-worktree-shell', always prompt for the agent."
      (interactive)
      (let ((agent-shell-preferred-agent-config nil)
            ;; Binds `agent-shell-preferred-agent-config' to nil so you pick an
            ;; agent (e.g. opencode, claude, codex) from
            ;; `agent-shell-agent-configs' instead of using the preferred one.
            )
        (call-interactively #'agent-shell-new-worktree-shell)))
    ...
    ;; Choose agent based on work/personal laptop.
    (agent-shell-preferred-agent-config (if my/work-machine-p 'claude-code 'opencode))
...
#+begin_src emacs-lisp
  (use-package gptel
    ...
    :bind (("<f7>" . (lambda () (interactive) (switch-to-buffer (gptel "*gptel chat*"))))
           :map gptel-mode-map
           ("C-c C-c" . (lambda () (interactive) (goto-char (point-max)) (gptel-send))))
