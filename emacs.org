...
- =Modified: Fang Lungang 2024-07-11T16:41:47+1000=
  ...
  #+begin_src org :tangle no
    # -*-mode:org;coding:utf-8;time-stamp-pattern:"8/Modified[:][ \t]*%U %Y-%02m-%02dT%02H:%02M:%02S%5z$"-*-
  #+end_src

  Note: =Modified[:]= matches =Modified:=. The reason using the former is to
  avoid the pattern itself being replaced
...
printing on A4 paper or for neat screen display. However, with the advent of
larger monitors, it's now possible to comfortably display two buffers, each with
100 characters.

#+begin_src emacs-lisp
(setq-default fill-column 100 comment-fill-column nil)
#+end_src
...
#+begin_src emacs-lisp
  (use-package copilot-chat
    ...
    :commands copilot-chat-display copilot-chat-how
    ...
    :config
    ;; Define my own shortcut(s). For one-off usage, run `copilot-chat-custom-prompt-selection`
    ;; instead.
