...
buffer's content if it contains emojis, particularly those using format
characters such as Zero Width Joiners (ZWJ) or Variation Selectors.
...
#+begin_src emacs-lisp
  (use-package copilot-chat
    ...
    ;; Note: Our company's subscription has monthly usage limits for most models;
    ;; `gpt-4.1' is unlimited. If you hit the limit, switch models temporarily
    ;; using `M-x copilot-chat-set-model'.
    (copilot-chat-default-model "claude-opus-4.5")
