...
#+AUTHOR: Fang Lungang
#+DATE: 2026-09-24 +1000
#+DESCRIPTION: My Emacs configurations
#+KEYWORDS: Emacs
#+PROPERTY: header-args:emacs-lisp :lexical yes :tangle ~/.emacs.d/init.el :mkdirp yes
...
- [[#start-up-screen-and-scratch-buffer][Start up screen and scratch buffer]]
  - [[#splash-screen-file][Splash screen file]]
- [[#language-environment-and-coding-system][Language environment and coding system]]
- [[#environment-variables][Environment Variables]]
- [[#emojis-and-format-characters-zero-width-joiners-variation-selectors][Emojis and Format Characters (Zero Width Joiners, Variation Selectors)]]
- [[#interaction-convenience][Interaction convenience]]
  - [[#mouse][Mouse]]
  - [[#y-or-n][y-or-n]]
  - [[#comment][Comment]]
  ...
  - [[#subword][Subword]]
  - [[#sentence][Sentence]]
- [[#shortcuts-to-files-and-links][Shortcuts to files and links]]
  - [[#ffap][ffap]]
  - [[#openwith-mode][Openwith mode]]
  - [[#webjump][Webjump]]
  - [[#custom-urls][Custom URLs]]
- [[#sessions-and-histories][Sessions and histories]]
...
- [[#files-and-directories][Files and directories]]
  ...
  - [[#dealing-with-huge-files][Dealing with huge files]]
  - [[#update-timestamps-before-save][Update timestamps before save]]
  - [[#directories][Directories]]
  - [[#uniquify-buffer-names][Uniquify buffer names]]
  - [[#buffer-file-path][Buffer file path]]
- [[#basic-editing][Basic editing]]
  - [[#fill][Fill]]
  - [[#whitespace][Whitespace]]
  - [[#yasnippet][Yasnippet]]
  - [[#find-and-replace-in-multiple-files][Find and replace in multiple files]]
  - [[#automatic-parenthesis-pairing][Automatic parenthesis pairing]]
- [[#spelling-check][Spelling check]]
  - [[#jinx][Jinx]]
- [[#visual-aids][Visual aids]]
  - [[#block-cursor-as-wide-as-the-glyph-under-it][Block cursor as wide as the glyph under it]]
  - [[#visual-bell][Visual Bell]]
  - [[#long-lines][Long lines]]
  - [[#display-column-number-in-the-mode-line][Display column number in the mode line]]
  - [[#display-line-numbers][Display line numbers]]
  - [[#clean-up-mode-line][Clean up mode line]]
  ...
  - [[#flash-cursor-on-windowbuffer-switch][Flash cursor on window/buffer switch]]
  - [[#highlight-custom-keywords][Highlight custom keywords]]
  - [[#focus-mode][Focus mode]]
  - [[#minimap][Minimap]]
- [[#finance-bookkeeping][Finance bookkeeping]]
  - [[#beancount][Beancount]]
  - [[#bookkeeping-snippets-for-beancount][Bookkeeping snippets (for beancount)]]
  - [[#bookkeeping-functions-for-beancount][Bookkeeping functions (for beancount)]]
- [[#encryption-and-credential-management][Encryption and credential management]]
  - [[#encrypt-files-using-keys-gnupg][Encrypt files using keys: GnuPG]]
  - [[#prompt-for-gpg-passphrases-in-the-minibuffer][Prompt for GPG passphrases in the minibuffer]]
  - [[#store-credentials-auth-source][Store credentials: auth source]]
...
- [[#org-mode][Org mode]]
  ...
  - [[#ticket-or-review-comments][Ticket or review comments]]
...
- [[#terminalshell][Terminal/shell]]
  - [[#erase-keybindings][erase keybindings]]
  - [[#vterm][vterm]]
  - [[#eshell][eshell]]
- [[#utilities][Utilities]]
...
- [[#ide][IDE]]
  - [[#treemacs][Treemacs]]
  - [[#syntax-check-flymake][Syntax check: flymake]]
  - [[#syntax-parser-tree-sitter][Syntax parser: tree-sitter]]
  ...
  - [[#dap-dape][DAP: dape]]
  - [[#xcscope][Xcscope]]
- [[#text-foldinghiding][Text folding/hiding]]
  - [[#hide-show][Hide show]]
...
- [[#json-json-lines][JSON, JSON Lines]]
  - [[#json-ts-mode][JSON ts mode]]
  - [[#formatter-1][Formatter]]
  - [[#structured-log-mode][Structured log mode]]
  - [[#json-path-to-the-node-at-point][JSON path to the node at point]]
- [[#latex][LaTeX]]
- [[#ldap-schema-ldif][LDAP Schema (LDIF)]]
- [[#markdown][Markdown]]
...
- [[#php][PHP]]
- [[#python][Python]]
  - [[#tree-sitter-enhanced-major-mode][Tree-sitter enhanced major mode]]
...
- [[#xml][XML]]
  - [[#nxml][nXML]]
  - [[#relax-ng-compact-rng--rnc][Relax-NG Compact (RNG & RNC)]]
- [[#yaml][YAML]]
  - [[#formatter-3][Formatter]]
  - [[#tree-sitter-enhanced-major-mode-1][Tree-sitter enhanced major mode]]
  - [[#yaml-pro][YAML pro]]
  - [[#flymake][Flymake]]
- [[#footer][Footer]]

* Timestamp for tangling                                           :noexport:
...
#+begin_src emacs-lisp
  (use-package use-package-core
    ...
    (use-package-compute-statistics t)    ; Profile start time
    )
#+end_src
...
These variables classify machines by their hostnames.
- =my/work-station-p= is non-nil on AWS development hosts whose names begin with
  =ip-10-=.
- =my/work-machine-p= is non-nil on both those workstations and work MacBooks,
  whose names begin with =M-=.

They are mainly used to conditionally tangle configuration files.

#+begin_src emacs-lisp
  (defconst my/work-station-regexp
    (rx (seq bos "ip-10-"))          ; work AWS dev hosts (e.g. this EC2 box)
    "Hostname regexp identifying my workstations.")
  (defvar my/work-station-p
    (and (string-match-p my/work-station-regexp (system-name)) t)
    "Is this my workstation?")
  (defconst my/work-machine-regexp
    (rx (or (seq bos "M-")          ; work MacBook
            (seq bos "ip-10-")))    ; work AWS dev hosts (e.g. this EC2 box)
    "Hostname regexp identifying my work machines.")
  (defvar my/work-machine-p
    (and (string-match-p my/work-machine-regexp (system-name)) t)
    "Is this my work machine (laptop or workstation)?")
#+end_src

NOTE: Avoid using them for conditional tangling within =emacs.org= itself: on a
fresh setup, they are not defined until the generated Emacs configuration has
been loaded, so a =:tangle= header that refers to them can fail with a
=void-variable= error. Use a runtime condition instead, as in the color theme
configuration below.
...
* Start up screen and scratch buffer
#+begin_src emacs-lisp
  (use-package emacs
    :custom
    (inhibit-startup-screen nil)
    (initial-buffer-choice (concat my-emacs-d "splash.org"))
...
This file is shown according to the value of ~initial-buffer-choice~ we set
above.

You can edit this file to include anything you find useful. For example:
+ Add reminders you want to see occasionally
+ Include shortcuts to start Claude Code (Agent Shell) in your active projects

An example is shown below.

#+begin_src org :tangle ~/.emacs.d/splash.org
  ,#+TITLE: Splash Screen
  ...
  - [[elisp:(let ((default-directory "~/source/project-a")) (agent-shell))][Claude @ A]]
  - [[elisp:(let ((default-directory "~/source/project-b")) (agent-shell))][Claude @ B]]
#+end_src
...
#+begin_src emacs-lisp
  (use-package exec-path-from-shell
    ...
    :demand t
    :config
    (dolist (var '("KUBECONFIG" "JIRA_API_TOKEN" "LIBGS"))
      (add-to-list 'exec-path-from-shell-variables var))
    (exec-path-from-shell-initialize))
#+end_src

* Emojis and Format Characters (Zero Width Joiners, Variation Selectors)

When running Emacs in a terminal, cursor movements may garble the display of the
buffer's content if it contains emojis involving combining characters, variation
selectors, or zero width joiners (ZWJ).

To prevent this issue, please disable ~auto-composition-mode~ as shown below.
...
NOTE:
- Emacs must be restarted because disabling auto composition on the fly does not
  work well.
- Trade-offs: emojis with ZWJ or variation selectors will render as multiple
  distinct characters rather than a single combined image.

See [[file:emacs-unicode-examples.org][my unicode examples]] for more information.
...
#+begin_src emacs-lisp
  (use-package emacs
    :bind (:map global-map
                ...
                ("C-c k" . my/kill-to-end-of-buffer))

    :custom
    ;; Send kills to the OS clipboard via OSC 52 (through tmux/ssh/wezterm).
...
Emacs kills can be copied directly to the OS clipboard by enabling OSC 52
selection support:
#+begin_src emacs-lisp
  (use-package emacs
    :custom
...
This works through tmux, SSH, and WezTerm. When using tmux, make sure clipboard
forwarding is enabled: ~set -g set-clipboard on~

To paste from the OS clipboard into Emacs, use the system paste shortcut
(for example, Cmd-V on macOS).

*** Canned responses

Combined with the MacOS clipboard integration, we can select a canned response
in Emacs and paste into any other applications.

#+begin_src emacs-lisp
  (use-package emacs
    :preface
    (defvar my/canned-responses
      '(("ack"          . "Acknowledged — looking into it now.")
        ("ooo"          . "I'm out today, will respond tomorrow.")
        ("thanks-ping"  . "Thanks for the ping — context loaded, replying shortly.")
        ("approve-merge". "LGTM, feel free to merge."))
      "Alist of (KEY . TEXT) for canned responses.")

    (defun my/canned-to-clipboard ()
      "Pick a canned response and push it to the macOS clipboard."
      (interactive)
      (let* ((key  (completing-read "Canned: " my/canned-responses nil t))
             (text (cdr (assoc key my/canned-responses))))
        (kill-new text)
        (message "Copied %s (%d chars)" key (length text))))

    :bind (("C-c y" . my/canned-to-clipboard)))
#+end_src
...
#+begin_src emacs-lisp
  (defconst my/ibuffer-regexp-llm-buffers
    (rx line-start
        (or
         (seq (* nonl) "Agent @" (* nonl))     ; Agent shell conversations
         (seq "acp-client-" (* nonl))          ; Agent shell ACP/debug buffers
         (seq "*claude-code" (* nonl) "*")     ; claude-code-ide Claude conversations
  ...
  (use-package ibuffer                    ; built-in package
    ...
    (ibuffer-saved-filter-groups
     `(("my"
        ...
        ("AI" (name . ,my/ibuffer-regexp-llm-buffers))
        ("Magit" (name . "^magit"))
        ("Emacs" (name . "^\\*.*\\*$"))
        )))
    ...
    :config
    ;; Set default view to detailed buffer names with unlimited width.
    ;; Press ` (backtick) to cycle through standard formats.
    (push '(mark modified read-only " " (name 30 -1 :left)) ibuffer-formats))
#+end_src
...
Vertico, orderless, marginalia, consult, corfu and cape form the core of the
modern, minimalist Emacs completion ecosystem. They are designed to work
together seamlessly while relying on standard Emacs APIs. Here is a summary of
their roles and usages:

- Minibuffer (Command Line) Packages:
  + Vertico :: The /UI for the minibuffer/. When you press =M-x= or search for a
    file, Vertico displays the available completion options in a vertical,
    interactive list.
  + Marginalia :: The /annotator for the minibuffer/. It adds helpful contextual
    information next to Vertico's list, such as function documentation, file
    sizes, or keybindings.
  + Consult :: The /search and navigation commands/. It provides advanced
    commands (like =consult-ripgrep=, =consult-line=, or =consult-buffer=) that
    feed rich data into the minibuffer for you to filter and select.
  + Orderless :: The /filtering engine/. It allows you to filter lists in the
    minibuffer (and in-buffer) by typing multiple search terms separated by
    spaces, in any order.

- In-Buffer (Text/Code) Packages:
  + Corfu :: The /UI for in-buffer completion/. It displays a small popup at
    your cursor to autocomplete code, variables, or text as you type.
  + Cape :: The /data provider for in-buffer completion/. It supplies extra
    lists of things to complete (like file paths, dictionary words, or text
    history) to be displayed by Corfu.
...
#+begin_src emacs-lisp
  (use-package vertico
    ...
    (vertico-resize nil)
    (vertico-cycle nil)
    ;; By default use a `buffer' to display the completion list & support mouse
    (vertico-multiform-categories '((t buffer mouse)))
    ;; The `buffer' is displayed at the bottom, with the given size
    (vertico-buffer-display-action '(display-buffer-at-bottom (window-height . 0.4)))
    (vertico-multiform-commands '(("gptel" grid mouse)))
    :init
    (vertico-mode 1)
  ...
  (use-package corfu
    :bind (:map global-map ("M-/" . completion-at-point))
    :custom
    (corfu-auto nil)                      ; disable auto popup; use M-/ to invoke manually
    :init
    (global-corfu-mode))
  ...
  (use-package cape
    :hook (agent-shell-mode . (lambda ()
                                ...
                                (add-hook 'completion-at-point-functions
                                          #'cape-dabbrev nil t)))
    :custom
    (cape-dict-file (expand-file-name "~/.emacs.d/en_AU.words"))
...
- Order cape functions :: the first function that returns candidates
  wins, so order them by priority in the list.
- Prepare cape-dict dictionary :: =cape-dict= reads =cape-dict-file=, whose
  default =/usr/share/dict/words= is US English on macOS and may be absent on
  Ubuntu. Generate an =en_AU= word list from aspell to match Jinx's dictionary:
  #+begin_src sh :tangle no
    aspell dump master en_AU | sort -u > ~/.emacs.d/en_AU.words
  #+end_src

** Embark

When I'm in a completion list from Vertico, Consult etc., Embark lets me say:
1. act on the current candidate
2. show me all actions available here
3. export these candidates to a normal buffer
4. perform actions by type (file, buffer, symbol, command, package, bookmark,
   etc.)

Currently, the most useful feature for me is exporting the candidates to a
normal buffer. With my configuration below, it is done via ~C-x .~ then ~S~.
...
#+begin_src emacs-lisp
  (use-package emacs
    ...
    (midnight-mode t)
    :custom
    ;; Ask for confirmation before killing Emacs. As I typically run the same
    ;; Emacs instance for days, an accidental `C-x C-c' would cost me the whole
    ;; session.
...
#+begin_src emacs-lisp
  (use-package recentf                    ; built-in package
    ...
    :custom
    (recentf-max-saved-items 66)
    (recentf-auto-cleanup 'never)
    :config
    (recentf-mode 1)
    (add-to-list 'recentf-keep 'file-remote-p)
    :bind
    (:map global-map ("<f1>" . recentf-open)))
#+end_src
...
Automatic cleanup is disabled: with =recentf-auto-cleanup= set to ='mode'= it
would run while enabling =recentf-mode= at startup and slow startup down, and a
periodic timer (a number) runs a full scan of the list that is rarely
worthwhile. Obsolete entries are left to age out on their own: the list is kept
in LRU order and truncated to =recentf-max-saved-items= each time it is saved,
so dead files sink to the tail as new files are opened. When I cannot wait for
that, =M-x recentf-cleanup= cleans the list by hand.
...
#+begin_src emacs-lisp
  (use-package emacs
    ...
    (when (display-graphic-p)
      (scroll-bar-mode -1)
      ;; Remove the title bar, use `(set-frame-parameter nil 'undecorated nil)` to
      ;; temporarily add the title bar back
  ...
  (use-package emacs
    :bind (:map global-map ("M-o" . other-window))
    :custom
...
Since Emacs 31, built-in commands cover all my window-management needs, so
=ace-window=, =windmove= and similar packages are no longer necessary for me.

- =M-o= or the default =C-x o= :: jump to the other window (=other-window=)
- =C-x w f= followed by <up>/<down>/<left>/<right> :: flip the windows
- =C-x w t= :: transpose the window layout
- =C-x w r <right>/<left>= :: rotate the whole layout clockwise/anticlockwise
- =C-x w o <right>/<left>= :: rotate the windows in cyclic ordering
  ...
  (use-package popper
    ...
    (popper-reference-buffers
     '("personal.gtd"
       ...
       "\\*\\(gptel chat\\|DeepSeek\\|.*GPT.*\\|LLM response\\)\\*" ; gptel
...
#+begin_src emacs-lisp
  (use-package emacs
    ...
    :custom
    (face-font-rescale-alist '(("SimSong" . 1.25) ("PingFang SC" . 1.25)))

    :config
    ;; font size: 160 = 16pt (1/10 pt units)
    (set-face-attribute 'default nil :height 160 :weight 'normal)
    ;; Default font: use the first available in the list
    (let ((mono-font (seq-find (lambda (f) (member f (font-family-list)))
                               '("FiraCode Nerd Font" "JetBrainsMono Nerd Font Propo"))))
      (when mono-font (set-face-attribute 'default nil :family mono-font)))
    ;; CJK font: use the first available in the list
    (let ((zh-font (seq-find (lambda (f) (member f (font-family-list)))
                              '("PingFang SC" "SimSong"))))
      (when zh-font (set-fontset-font t 'han zh-font)))
    ;; ;; Emoji font
...
NOTE: To fully utilize color themes in text-based Emacs, make sure your terminal
is set up to support "true color". See the [[file:shell.org::*Term & color][Term & color]] section in my shell
configuration file for the details

** Themes

I use a different theme on my workstation so the color scheme also serves as a
visual cue for which environment I'm in.

The themes I choose are low-contrast ones that are easy on the eyes.

#+begin_src emacs-lisp
  (use-package solarized-theme
    :if (not my/work-station-p)
    :demand t
    :config
    (load-theme 'solarized-gruvbox-dark t))

  (use-package doom-themes
    :if my/work-station-p
    :demand t
    :config
...
- Removes the background and foreground highlighting from flymake warnings, so
  that only the underline (and similar decorations) are used to indicate
  warnings. The default colored backgrounds are too distracting for my taste.
- Removes the background colors from Magit diff faces. Some themes (e.g.
  solarized) set loud background colors on diff hunks that I find visually
  noisy; clearing them lets the foreground colors carry the diff semantics.
...
- =Modified: Fang Lungang 2024-07-11T16:41:47+1000=
  ...
  #+begin_src org :tangle no
    # -*-mode:org;coding:utf-8;time-stamp-pattern:"8/Updated[:][ \t]*%L %Y-%02m-%02dT%02H:%02M:%02S%5z$"-*-
  #+end_src
...
side, each with 100 characters, there are still times we work on the built-in
...
Snippets for annotating files with notes for AI agents.

NOTE: these are defined in elisp instead of as snippet files in
=~/.emacs.d/snippets/text-mode/= on purpose: org-babel tangling always ends the
tangled file with a newline, and, so file-based snippets would insert an
unwanted trailing newline on every expansion. =yas-define-snippets= gives exact
control over the template string.

#+begin_src emacs-lisp
  (with-eval-after-load 'yasnippet
    (yas-define-snippets 'text-mode
      '(("todo" "[TODO: $1]" "TODO note")
        ("ask"  "[ASK: $1]"  "ASK note"))))
#+end_src
...
This is a built-in package that I no longer use. I now use Jinx for its better
user interface.
...
Jinx is a 3rd party replacement of Flyspell. It comes pre-configured for the
major modes used below (prog, text, org, ...), so enabling =global-jinx-mode= is
sufficient.

Jinx uses Enchant, which supports multiple backends (hunspell, aspell, etc.).
While hunspell is generally recommended, on macOS aspell is often easier because
hunspell does not ship dictionaries and there are no Homebrew packages for them.

#+begin_src emacs-lisp
  (use-package jinx
    :delight
    :hook (emacs-startup . global-jinx-mode)
    :bind (("<f6>" . jinx-correct)
           ("M-$" . jinx-correct)
           ("C-M-$" . jinx-languages))
    :custom
    (jinx-languages "en_AU")
    :config
    ;; YAML files: spell-check comments only, not string values
    (setf (alist-get 'yaml-mode jinx-exclude-faces) '(font-lock-string-face)
          (alist-get 'yaml-ts-mode jinx-exclude-faces) '(font-lock-string-face))
    ;; Chinese characters: Jinx uses Emacs syntax tables to identify words.
    ;; Because CJK characters have word syntax by default, Jinx treats them as
    ;; words and flags every character as misspelled. Since dictionary-based
    ;; spell-checking is not applicable to individual CJK characters, classify
    ;; them as non-word characters.
    (let ((st jinx--syntax-table))
      (modify-syntax-entry '(#x4E00 . #x9FFF) "_" st)   ; CJK Unified Ideographs
      (modify-syntax-entry '(#x3400 . #x4DBF) "_" st)   ; CJK Extension A
      (modify-syntax-entry '(#x20000 . #x2A6DF) "_" st) ; CJK Extension B
      (modify-syntax-entry '(#x2A700 . #x2B73F) "_" st) ; CJK Extension C
      (modify-syntax-entry '(#x2B740 . #x2B81F) "_" st) ; CJK Extension D
      (modify-syntax-entry '(#x2B820 . #x2CEAF) "_" st) ; CJK Extension E
      (modify-syntax-entry '(#x2CEB0 . #x2EBEF) "_" st) ; CJK Extension F
      (modify-syntax-entry '(#x30000 . #x3134F) "_" st) ; CJK Extension G
      (modify-syntax-entry '(#x31350 . #x323AF) "_" st) ; CJK Extension H
      (modify-syntax-entry '(#x2EBF0 . #x2EE5F) "_" st) ; CJK Extension I
      ))
#+end_src
...
** Flash cursor on window/buffer switch

When switching windows or buffers, it is easy to lose track of where the
cursor landed. ~beacon-mode~ briefly flashes a light ring at the cursor
position, making the new location immediately obvious.
...
#+begin_src emacs-lisp
  (use-package font-lock
    ...
    (defun my/highlight-keywords ()
      ;; Keywords cover the semantic range: FIXME (broken), TODO (pending),
      ;; NOTE (info), ASK (question for AI agents), lgfang (personal
      ;; annotations).
      (font-lock-add-keywords
       nil
       '(("\\<\\(lgfang\\|TODO\\|FIXME\\|NOTE\\|ASK\\)\\>"
          . (0 font-lock-warning-face t)))))
    :hook
    (font-lock-mode . my/highlight-keywords)
    :config
    (font-lock-add-keywords
     'c-mode
...
#+begin_src emacs-lisp
  (use-package evergreen
    ...
    (evergreen-patch-default-flags '("--include-modules" "--uncommitted"))
    )
#+end_src
...
That said, on MacOS, GnuPG must be configured properly, otherwise, you may see
the following error:
#+begin_src text
...
See [[file:macos.org::*Setup GnuPG (GPG)][Setup GnuPG (GPG)]] for the details
...
** Prompt for GPG passphrases in the minibuffer

Have GnuPG hand passphrase prompts back to Emacs (the minibuffer) instead of
spawning an external pinentry. In =emacs -nw= under tmux, the external pinentry
may draw on a TTY that is not visible, leaving Emacs frozen while gpg waits for
input that can never arrive. (gpg-agent allows loopback pinentry by default
since GnuPG 2.1.12.)
#+begin_src emacs-lisp
  (use-package epg
    :ensure nil                           ; Built-in
...
#+begin_src emacs-lisp
  (use-package elfeed
    ...
    :bind (:map elfeed-search-mode-map
                ("SPC" . elfeed-search-untag-all-unread))
    :custom
    (elfeed-search-title-max-width 120)
    (elfeed-search-filter "@1-weeks-ago +unread")
    (elfeed-feeds
     '(;; Emacs
       ("https://www.reddit.com/r/emacs.rss" en emacs)
       ("https://emacs-china.org/latest.rss" zh emacs)

       ;; Tech
       ("https://hnrss.org/frontpage" en tech news hn)

       ;; Chinese & World News
       ("https://plink.anyfeeder.com/newscn/whxw" zh news xinhua)
       ("https://plink.anyfeeder.com/zaobao/realtime/china" zh news zaobao)
       ("https://plink.anyfeeder.com/zaobao/realtime/world" zh news zaobao)

       ;; Australia News
       ("https://www.abc.net.au/news/feed/51120/rss.xml" en news abc)
       ("https://www.theguardian.com/australia-news/rss" en news guardian)
       ("https://www.sbs.com.au/news/feed" en news sbs)
       ("https://www.smh.com.au/rss/feed.xml" en news smh)
       ("https://theconversation.com/au/articles.atom" en conversation)
       ("https://www.news.com.au/national/rss" en news newscorp))
     ))
#+end_src
...
** COMMENT News groups and emails - gnus                                   :obsolete:
...
#+begin_src emacs-lisp
  (use-package kubel
    :commands kubel
    :custom
    (kubel-default-namespace "devprod-platform"))
#+end_src
...
#+begin_src emacs-lisp
  (use-package org
    :ensure nil                           ; Built-in
    :custom
    ;; Todo keywords: !@ means attach timestamps/notes when changing to certain state
    (org-todo-keywords
     '((type "INBX(i!)" "TODO(t!)" "PROG(p!)" "WAIT(w@)" "HOLD(h!)" "MAYB(m!)"
             "|" "DONE(d!)" "DROP(c@)" "PASS(o@)")))

    ;; Todo dependencies
    (org-enforce-todo-dependencies t)     ; Block DONE until dependencies are done
    (org-agenda-dim-blocked-tasks 'invisible) ; Hide blocked tasks in agenda

    ;; Refile targets: mark possible targets with the tag "headline"
    (org-refile-targets '((org-agenda-files . (:tag . "headline"))))

    ;; Logging
    (org-log-done 'time)
    (org-log-into-drawer t)              ; log state changes in 'LOGBOOK' drawer
    ...
    ;; Default notes file (overwritten by org-capture-templates)
    (org-default-notes-file "~/.income.org")

    :config
    (defun my/skip-unless-worked-on (&optional days)
      "Skip unless worked on within the last DAYS days (default 2).

  An entry counts as worked on if it was closed within DAYS days, or
  had a LOGBOOK state transition to or from PROG within DAYS days.
  Entries currently in PROG are not handled here; the companion
  `todo' PROG block of the report lists them regardless of when they
  started."
      (let ((end (save-excursion (outline-next-heading) (point)))
            ...
            (cutoff (format-time-string
                     ...
                     (time-subtract (current-time)
                                    (days-to-time (or days 2)))))
            found)
        (save-excursion
          (while (and (not found)
                      (re-search-forward
                       "^\\(?: *CLOSED: *\\| *- State \"\\(\\w+\\)\" +from \"\\(\\w+\\)\" +\\)\\[\\([0-9-]+\\)"
                       end t))
            (let ((date (match-string 3))
                  (to (match-string 1))
                  (from (match-string 2)))
              (when (and (string> date cutoff)
                         (or (null to)            ; a CLOSED line
                             (equal to "PROG")    ; entered PROG
                             (equal from "PROG"))) ; left PROG
                (setq found t)))))
        (if found nil (point-at-eol))))

    (defun my/report-worked-on-tasks (days)
      "Show tasks worked on in the last DAYS days (default 2).

  An entry is included if it is currently PROG, was closed within
  DAYS days, or had a state transition to or from PROG within DAYS
  days.  The first block lists entries still in PROG; the second
  lists the closed/recently-transitioned ones."
      (interactive
       (list (cond ((null current-prefix-arg) 2)
                   ((integerp current-prefix-arg) current-prefix-arg)
                   (t (read-number "Number of days: " 2)))))
      (let* ((header (format "Worked on (last %s days)" days))
                  ...
                  (tags "/-PROG"
                        ((org-agenda-overriding-header "")
                         (org-agenda-skip-function
                          '(my/skip-unless-worked-on ,days)))))))))
        (org-agenda nil "r")))

    (defun my/agenda-skip-scheduled-no-deadline ()
      "Skip scheduled entries that have no deadline.

       Scheduled tasks without a deadline are displayed in the `todo'
       sections of the custom agenda commands instead.  Entries with a
       deadline are kept. Appointments (plain timestamps)
       are unaffected and display in the `agenda' section."
      (unless (org-entry-get (point) "DEADLINE")
        (org-agenda-skip-entry-if 'scheduled)))

    ;; Settings that depend on my-personal-path (evaluated at load time)
    (setq org-agenda-files
          (mapcar (lambda (x) (concat my-personal-path x))
                  '("work.gtd" "personal.gtd" "part-time.gtd")))

    (setq org-archive-location (concat my-personal-path "archive.gtd::* From %s"))

    (setq org-capture-templates
          `(("w" "work related" entry
             (file+olp ,(concat my-personal-path "work.gtd") "Misc.")
             "* INBX %?" :prepend t)
            ("p" "personal task" entry
             (file+olp ,(concat my-personal-path "personal.gtd") "Misc.")
             "* INBX %?" :prepend t)))

    (setq org-agenda-custom-commands
          `(("x" "TODO items @ALL@"
             ((todo "INBX" ((org-agenda-todo-ignore-scheduled 'future)))
              (todo "PROG" ((org-agenda-todo-ignore-scheduled 'future)))
              (todo "TODO" ((org-agenda-todo-ignore-scheduled 'future)))
              (agenda "" ((org-agenda-span 2)
                          (org-deadline-warning-days 7)
                          (org-agenda-skip-function 'my/agenda-skip-scheduled-no-deadline)))))
            ("y" "TODO items @WORK@"
             ((todo "INBX"
                    ((org-agenda-files '(,(concat my-personal-path "work.gtd")))
              ...
              (agenda ""
                      ((org-agenda-span 2)
                       ...
                       (org-agenda-skip-function 'my/agenda-skip-scheduled-no-deadline)))))
            ("p" "PENDING items"
             ((todo "WAIT") (todo "HOLD") (todo "MAYB")))
            ("r" "Report worked on tasks"
             (lambda (_) (call-interactively #'my/report-worked-on-tasks)))
            ("b" "My bookmark" search nil
             ((org-agenda-files '(,(concat my-personal-path "bookmarks.lgf")))))
...
If using karthink's org-mode fork (see below):
#+begin_src emacs-lisp
  (use-package org-latex-preview
    :if (locate-library "org-latex-preview")
    :ensure nil
    :after org
    :config
    (setq org-latex-preview-process-default 'dvisvgm)
    (setq org-latex-preview-cache "/tmp/ltximg/")
    (setq org-latex-preview-appearance-options
          (plist-put org-latex-preview-appearance-options :zoom 1.2)))
#+end_src

Otherwise, using the built-in org-mode:
#+begin_src emacs-lisp
  (use-package org
    :unless (locate-library "org-latex-preview")
    :config
...
*** Optional: live preview with karthink's org-mode fork

To get automatic live rendering of LaTeX fragments as you type, you can use
[[https://github.com/karthink/org-mode][karthink's org-mode fork]] (branch ~olp~) which provides ~org-latex-preview-mode~.
When the cursor enters a LaTeX fragment, the source code is shown for editing;
when the cursor leaves, the rendered preview is restored automatically.

The feature has been under review on the org-mode mailing list since 2023
("[Pre-PATCH] Overhaul of the LaTeX preview system") but is not yet merged
upstream. When it merges, remove the fork and revert to the built-in org.

Install the fork:
#+begin_src sh
  git clone https://github.com/karthink/org-mode --branch olp ~/source/org-mode
  cd ~/source/org-mode && make autoloads compile
#+end_src

Then create =~/.emacs.d/early-init.el= to load the fork instead of the
built-in org:
#+begin_src emacs-lisp :tangle ~/.emacs.d/early-init.el :tangle no
  ;;; -*- lexical-binding:t; time-stamp-pattern:"8/#\\+DATE:[ \t]+%Y-%02m-%02d %5z$" -*-
...
Convert an org file to HTML reveal slides. Prefer =org-tree-slide= for live presentations.
#+begin_src emacs-lisp
...
The ~toc-org-mode~ minor mode ensure the first header with the tag ~:TOC:~ have
...
I need sentence/paragraph translation more than single word lookup.

#+begin_src emacs-lisp
  (use-package gt
    :bind ("C-h d" . gt-translate)
    :config
    (setq gt-langs '(en zh-CN))
...
#+begin_src emacs-lisp
  (use-package breadcrumb
    ;; Track upstream master: the stable ELPA release (1.0.1, Sep 2023) predates
    ;; the fix for dead imenu markers in org buffers (github#22).
    ...
    :config
    (breadcrumb-mode 1))                  ; enable the mode globally
#+end_src
...
- ACPs are installed
- Corresponding API keys stored in Emacs auth sources

#+begin_src emacs-lisp
  (use-package agent-shell
    :preface
    (defvar my/agent-shell-dir-history nil
      "History of directories chosen by `my/agent-shell'.")

    (defun my/agent-shell (&optional arg)
      "A wrapper of `agent-shell' and `agent-shell-new-shell'.
  Without prefix, run `agent-shell'. With prefix ARG (C-u), select a
  directory and run `agent-shell-new-shell' there. Offers the
  sub-directories of ~/source and the current directory, but also allows
  entering any path with directory-name autocompletion."
      (interactive "P")
      (if (not arg)
          (agent-shell)
        (let* ((dirs (mapcar #'abbreviate-file-name
               ...
               (chosen-dir
                (completing-read
                 "Select agent-shell directory: "
                 (completion-table-merge dirs #'completion-file-name-table)
                 #'file-directory-p
                 nil nil 'my/agent-shell-dir-history))
               (default-directory (expand-file-name chosen-dir)))
          (agent-shell-new-shell))))

    (defun my/anthropic-model-id-from-name (name)
      "Return a function that resolves model NAME to the latest model ID."
      (lambda ()
        (car (sort (seq-keep (lambda (model)
                               (when (string-match-p (downcase name)
                                                     (downcase (or (map-elt model :name) "")))
                                 (map-elt model :model-id)))
                             (agent-shell--get-available-models agent-shell--state))
                   #'string-greaterp))))

    :bind (("C-c a a" . my/agent-shell)
           ("C-c a w" . agent-shell-new-worktree-shell)
           ("C-c a r" . agent-shell-send-region-to)
           :map agent-shell-mode-map
           ;; Use RET for newline, C-c RET to submit: not C-c C-c, because most AI
           ;; agent CLI tools (opencode, claude, etc.) use C-c C-c to quit, so
           ;; binding it here causes accidental quits due to muscle memory.
           ("RET" . agent-shell-newline)
           ("C-c RET" . shell-maker-submit))

    :custom
    ;; Use single-frame busy indicator so that it emits no extra terminal output
    ;; while thinking. Under tmux this means the window is flagged as active only
    ;; when real output appears, not the moment the agent starts thinking.
    (agent-shell-busy-indicator-frames '("⏳Thinking..."))

    (agent-shell-session-strategy 'prompt)

    ;; Blocking sleep on macOS only. Do that on headless workstations makes no
    ;; sense and fails with AccessDenied on every agent turn.
    (agent-shell-inhibit-system-sleep (eq system-type 'darwin))

    ;; Preselect an agent based on work/personal laptop, but keep the picker so
    ;; another agent can still be chosen.
    (agent-shell-preferred-agent-config
     (cons 'preselect (if my/work-machine-p 'claude-code 'opencode)))

    ;; Anthropic
    (agent-shell-anthropic-default-model-id (my/anthropic-model-id-from-name "glm.*flash.*latest"))
    ...
    (agent-shell-openai-codex-environment
     (when my/work-machine-p
       (list (format
              "GROVE_API_KEY=%s"
              (or (auth-source-pick-first-password :host "grove" :user "code-gen-api")
                  (error "API key (code-gen-api@grove) missing in auth-source"))))))
    (agent-shell-openai-authentication
     (agent-shell-openai-make-authentication
      ;; Placeholder only — satisfies codex-acp's OPENAI_API_KEY presence check.
      ;; Real auth comes from ~/.codex/config.toml, using GROVE_API_KEY injected
      ;; via agent-shell-openai-codex-environment.
      :api-key "grove-placeholder-real-key-from-config-toml"))

    ;; OpenCode
    (agent-shell-opencode-default-model-id
     ;; choose model based on work/personal laptop
     (if my/work-machine-p
...
An add-on to ~agent-shell~ that provides a peek interface for managing
multiple ~agent-shell~ sessions, organised by project.

#+begin_src emacs-lisp
  (use-package persp-mode
    ;; Install and turn on persp-mode for agent-shell-hq-toggle
    :delight
    :config
    (persp-mode 1))
  ...
  (use-package agent-shell-hq
    :vc (:url "https://github.com/SreenivasVRao/agent-shell-hq"
        :rev :newest)
    :commands (agent-shell-hq-peek agent-shell-hq-label agent-shell-hq-toggle agent-shell-hq-toggle-jump-to-sidebar)
    :bind (("C-c a p" . agent-shell-hq-peek)
           ("C-c a s" . agent-shell-hq-toggle-jump-to-sidebar))
    :custom
...
#+begin_src emacs-lisp
  (use-package gptel
    ...
    :bind (("<f7>" . (lambda () (interactive) (switch-to-buffer (gptel "*gptel chat*"))))
           :map gptel-mode-map
           ;; C-c RET jumps to buffer end first, so all text is included in the prompt.
           ("C-c RET" . (lambda () (interactive) (goto-char (point-max)) (gptel-send))))
    ...
    ;; Don't show the reasoning ("thinking") blocks some models may spit
    (gptel-include-reasoning nil)

    :preface
    (defun my/gptel-get-key (host login)
      "Return a lambda that fetches the API key for HOST and LOGIN from auth-source.

         Returns a lambda not the results so the actual secret is looked up
         only when gptel needs it, not when Emacs is initializing."
      (lambda ()
        (require 'auth-source)
        (let* ((result (auth-source-search :host host :user login :require '(:secret)))
               (entry (car result)))
          (if entry
              (let ((secret (plist-get entry :secret)))
                (cond
                 ...
                 (t (error "Secret for %s@%s is not a string or function" login host))))
            (error "Failed to retrieve API key for %s@%s" login host)))))
    ...
    (let (
          (deepseek
           (gptel-make-deepseek "DeepSeek"
             :stream t
             :key (my/gptel-get-key "deepseek" "api")))

          (gemini
           (gptel-make-gemini "Gemini"
             :stream t
             :key (my/gptel-get-key "gemini" "free-api")))
          ...
          ;; (copilot (gptel-make-gh-copilot "CopilotChat"))

          ;; (mongogpt
          ;;  (gptel-make-openai "MongoGPT"
          ;;    :stream nil
          ;;    :host "mongogpt.aix.prod.corp.mongodb.com"
          ;;    :endpoint "/api/v1/messages"
          ;;    :key (lambda ()
          ;;           (let* ((key (string-trim
          ;;                        (shell-command-to-string "kanopy-oidc login")))
          ;;                  (jwt-pattern "^[A-Za-z0-9_-]+\\.[A-Za-z0-9_-]+\\.[A-Za-z0-9_-]+$"))
          ;;             (if (string-match-p jwt-pattern key)
          ;;                 key
          ;;               (error "Failed to retrieve API key: [%s]" key))))))

          (fireworks
           ;; Fireworks' OpenAI-compatible API, accessed directly. The
           ;; auth-source entry distinguishes laptop/workstation keys:
           ;; machine fireconnect login laptop|workstation
           ;;
           ;; :models must be explicit: gptel-make-openai otherwise defaults
           ;; to OpenAI's public catalog, which does not include
           ;; account-scoped Fireworks models like the GLM ones.
           (gptel-make-openai "Fireworks"
             :stream t
             :host "api.fireworks.ai"
             :endpoint "/inference/v1/chat/completions"
             :models '(accounts/fireworks/routers/glm-flash-latest)
             :key (my/gptel-get-key "fireconnect"
                                    (if my/work-station-p "workstation" "laptop"))))

          (grove
           (gptel-make-openai "Grove"
             :stream nil
             :host "grove-gateway-prod.azure-api.net"
             :endpoint "/grove-foundry-prod/openai/v1/chat/completions"
             :key (my/gptel-get-key "grove" "code-gen-api")
             :header (lambda (_info)
                       `(("api-key" . ,(funcall (my/gptel-get-key "grove" "code-gen-api")))))
             )))

      (if my/work-machine-p
          (progn
            (setq-default gptel-backend fireworks)
            (setq-default gptel-model 'accounts/fireworks/routers/glm-flash-latest))
        (progn
...
like this:
#+begin_src text
machine deepseek login api password sk-*******
#+end_src
...
#+begin_src emacs-lisp
  (use-package magit
    ...
    ;; Use the standard mechanism (configurable via `display-buffer-alist')
    ;; instead of forcing reuse of the same window.
...
#+begin_src emacs-lisp
  (use-package git-link
    :custom
    (git-link-open-in-browser (not my/work-station-p))
...
This configuration serves two scenarios:

- Linking to code on a local, unpushed branch

  Because the commit is unavailable remotely, its permalink would be broken. Run
  ~M-x git-link~ to link to the file on the default branch (~master~). The URL
  is copied to the kill ring and, except on my workstation (assumed to have no
  browser), opened so I can find the designated lines and create a permalink.

- Linking to code on a pushed commit

  Run ~C-u C-u M-x git-link~ to copy a permalink containing the commit hash to
  the kill ring.
...
#+begin_src emacs-lisp
  ...
  ;; A primitive minor mode to view log files containing ANSI color codes. It
  ;; runs `ansi-color-apply-on-region' lazily, i.e. only the visible part of
  ;; the buffer. Hence, it does NOT freeze Emacs even if the log file is huge.
  ;;
  ;; Cons: a) toggling the mode off does not undo what has already been
  ;; colorized. b) assumes the buffer content does not change. c) jumping to
  ;; random places within the buffer may incur incorrect/incomplete
  ;; colorization.
  (defun my/ansi-color-after-scroll (window start)
    "Apply ANSI color from START to the end of WINDOW.

    Not run manually but added to `window-scroll-functions' when
    `my/ansi-color-minor-mode' is turned on."
    (let ((ansi-color-context-region nil))
      (ansi-color-apply-on-region start (window-end window t) t)))

  (define-minor-mode my/ansi-color-minor-mode
    "A simple minor mode that translates and render ANSI color codes."
    :global nil
    :lighter ""
    (if my/ansi-color-minor-mode
        (progn
          (require 'ansi-color)
          (ansi-color-apply-on-region (window-start) (window-end) t)
          (add-hook 'window-scroll-functions #'my/ansi-color-after-scroll 80 t))
      (remove-hook 'window-scroll-functions #'my/ansi-color-after-scroll t)))
#+end_src
...
=csv-align-mode= pads columns with whitespace so fields line up vertically, easy
to scan, but wide tables can exceed the window width. =rainbow-csv-mode=
colourises each column instead, keeping lines short while still distinguishing
fields visually. Both are enabled below; toggle =csv-align-mode= off if a table
is too wide.
...
#+begin_src emacs-lisp
  (use-package structured-log-mode
    :vc (:url "https://github.com/lgfang/structured-log-mode"
...
Firstly, ensure TeX tools are installed. On macOS, install ~mactex-no-gui~,
which provides the full TeX Live distribution needed for preview rendering and
PDF export, without the GUI applications bundled in the full ~mactex~ cask.
#+begin_src sh
  brew install --cask mactex-no-gui
#+end_src

Then, write LaTeX directly with the ~auctex~ package, or embed LaTeX snippets in
org files and enable ~org-latex-preview-mode~ for live preview.
...
The built-in ~markdown-ts-mode~ (Emacs 32+) supersedes the obsolete
~markdown-mode~. It is now the default major mode for =.md= files.

#+begin_src emacs-lisp
  (use-package markdown-ts-mode
    :mode "\\.md\\'" ; remove when all my Emacs are 32+, where the .md association is built-in
...
I find ~markdown-ts-mode~'s colours normally look more comfortable. But
~md-mode~ can be useful in some cases:
- View mode enables single-key navigation, ideal for reading (most of my
  Markdown time is reading, not editing).
- Correct wrapping of bullet lists and table cells.
- LaTeX math equation rendering.

Therefore, install this package but do not set it as the default major mode for
Markdown.
#+begin_src emacs-lisp
  (use-package md-mode
    ...
    :hook ((md-mode . visual-line-mode)
           (md-mode . md-mode-render)))
#+end_src

** Preview

These are rarely needed: ~md-mode~'s view mode and ~markdown-ts-mode~'s
~markdown-ts-hide-markup~ already render the content well enough for reading.

*** StackEdit (online)

1. Open https://stackedit.io/ with a web browser
2. Click "START WRITING" bar at the top of the page
3. Paste the content into the editing area (left side)

NOTE: It can be used to preview org mode buffer as well: export the buffer to
Github Flavoured Markdown first using ~M-x org-gfm-export-as-markdown~.
...
#+begin_src emacs-lisp
  (use-package grip-mode
    ...
    :custom
    (grip-github-user "your-user-name")
    (grip-github-password (auth-source-pick-first-password
                           :host "api.github.com"
                           :user "your-user-name^grip")))
#+end_src

**** Privacy note

IMPORTANT: By default grip sends the Markdown to GitHub's rendering API so the output
matches GitHub exactly. That means the file contents leave our machine.

**** Prerequisites:
  - Install the command line tool ~grip~ via ~pipx install grip~.
  - Store GitHub PAT in Emacs auth source, if you want to
    - Avoid GitHub markdown API rate limit
    - Access private repository context (?)
