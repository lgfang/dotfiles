...
1. Run =grep-find= as usual.
2. In the output buffer, run =C-x C-p= to enter =wgrep= mode.
...
#+begin_src emacs-lisp
  (use-package copilot-chat
    ...
    :commands (copilot-chat--prepare-buffers
               copilot-chat-how
  ...
  (use-package minuet
    ...
    (let ((auth (auth-source-search :host "deepseek" :require '(:secret))))
      (if auth (setenv "DEEPSEEK_API_KEY" (funcall (plist-get (car auth) :secret)))
        (error "DeepSeek API key not found in your auth source")))
