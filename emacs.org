...
#+begin_src emacs-lisp
  (use-package dired
    ...
    :bind (:map dired-mode-map ("`" . dired-hide-details-mode)))
#+end_src
...
#+begin_src emacs-lisp
  (use-package emacs
    ...
    (defun my/current-file-path ()
      "Copy current file path to the kill ring.
If the buffer is not visiting a file, copy its default directory instead."
      (interactive)
      (let ((string (or (buffer-file-name)
                        (expand-file-name default-directory))))
        (message "Current file: %s" string)
        (kill-new string)))
    )
#+end_src
  ...
  (use-package elfeed
    ...
    (elfeed-feeds
     '(;; Emacs
       ...
       ;; Tech
       ("https://hnrss.org/frontpage" en tech hn)

       ;; Chinese news
       ("http://feeds.sbs.com.au/sbs-mandarin" zh news sbs)
       ;; Australia News
