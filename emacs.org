...
*** Prerequisite: Setup Claude Code (with AWS Bedrock)
...
#+begin_src emacs-lisp
  ;; Prerequisites for agent-shell with Claude
  ;; - Install claude: brew install claude-code
  ;; - Install claude-code-acp: npm install -g @zed-industries/claude-code-acp
