...
NOTE: =cape-dict= reads =cape-dict-file=, whose default
=/usr/share/dict/words= is US English on macOS and may be absent on Ubuntu.
Generate an =en_AU= word list from aspell to match Jinx's dictionary:
...
#+begin_src emacs-lisp
  ...
  (use-package agent-shell-hq
    :vc (:url "https://github.com/lgfang/agent-shell-hq"
        :rev :newest)
