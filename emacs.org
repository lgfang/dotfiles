  ...
  (use-package webjump                    ; built-in package
    ...
    (webjump-sites '(("google"
                     ...
                     ("jira my-closed"    ; Tickets I closed in the last 7 days
                      . (concat "https://jira.mongodb.org/issues/?"
                                "jql=status%20changed%20to%20Closed%20by%20currentUser()%20after%20-7d"))
                     )))
#+end_src
...
#+begin_src emacs-lisp
  ...
  The format is determined by the buffer name prefix."
      (interactive)
      (my/mark-comment)
      (let ((my/get-relative-level
             (lambda (headline info)
               ;; A temporary function to adjust headline levels for export. Start
               ;; headlines at level 3 (h3), as h1/h2 are often too large in Jira
               ;; tickets.
               (+ 2
...
#+begin_src emacs-lisp
  (use-package gptel
    :vc (:url "https://github.com/lgfang/gptel" :rev :newest)
    ...
    (eval-after-load 'gptel-bedrock
      '(progn
         (push '(haiku35 . "us.anthropic.claude-3-5-haiku-20241022-v1:0") gptel-bedrock--model-ids)
         (push '(sonnet4 . "us.anthropic.claude-sonnet-4-20250514-v1:0") gptel-bedrock--model-ids)))

    (gptel-make-bedrock "Bedrock"
      :stream t
      :region "us-east-1"
      :models '(haiku35 sonnet4)
      :profile "ai-coder-llm"))
#+end_src
...
*NOTE:* This section contains configurations for multiple packages for Claude
Code. Choose and activate only one of them, as they use the same hotkeys.
Enabling multiple packages simultaneously will cause keybinding conflicts.

*** Claude Code setup (with AWS Bedrock)

1. Install the Claude Code command line tool
   + Following the [[https://docs.anthropic.com/en/docs/claude-code/overview#get-started-in-30-seconds][official documentation]].
   + See also [[file:ai.org::*User-Specific "Global" Installation][User-Specific "Global" Installation]].

2. Configure Claude to use AWS Bedrock. For example, add the following to your
   =~/.claude/settings.json=. See [[file:ai.org::*Claude][my claude notes]] for more information.
   #+begin_src json-ts
     {
       "env": {
...
#+begin_src emacs-lisp :tangle no
  (use-package claudemacs
    ...
    :delight
    :bind (:map global-map ("C-c c" . claudemacs-transient-menu)))
#+end_src
