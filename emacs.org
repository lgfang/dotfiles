...
#+begin_src emacs-lisp
  (use-package emacs
    ...
    :bind (:map global-map
                ("<f8>" .
                 (lambda() "hide current buffer and try deleting window."
                   (interactive) (ignore-errors (bury-buffer) (delete-window)))))
    )
...
To create an additional eshell buffer, rename the existing ones first and then
run =M-x eshell=.

If you frequently need multiple eshell buffers, consider using =multi-eshell=
package. This package creates a new eshell buffer named =*eshell*<N>= for each
invocation of =M-x multi-eshell=.
