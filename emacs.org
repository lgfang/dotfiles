...
#+begin_src emacs-lisp
  (use-package agent-shell
    ...
    (defun my/agent-shell (&optional arg)
      "A wrapper of `agent-shell'. With prefix ARG (C-u), select the directory.
  Offers the subdirectories of ~/source and the current directory, but
  also allows entering any path with directory-name autocompletion."
      (interactive "P")
      (if arg
