  ...
  (use-package elfeed
    ...
    (elfeed-feeds
     '(;; Emacs
       ...
       ;; Social network hot
       ("https://rsshub.rss3.workers.dev/weibo/search/hot" zh social weibo)
...
This is intended for quick word lookup. I chose ~maple-translate~ because it is
simple and self-contained, and is sufficient for this purpose. Its only downside
is that it requires internet access.

=scdv= was not chosen, even though it works with local dictionary files, because
such dictionary files are now difficult to find.

#+begin_src emacs-lisp
  (use-package emacs-maple-translate
    :vc (:url "https://github.com/honmaple/emacs-maple-translate" :rev :newest)
