...
~markdown-mode~. Not currently in use (~:tangle no~).
...
My preferred Markdown mode. Advantages over ~markdown-ts-mode~:
- View mode with single-key navigation, ideal for reading (most of my Markdown
  time is reading, not editing).
- Correct wrapping of bullet lists in ~visual-line-mode~.
- Rendering style that I prefer.
