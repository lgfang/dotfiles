...
Note that there is normally no need of setting =repeat-eixt-key= or
=repeat-exit-timeout=. To exit repeat mode, just type any character which is no
bound in this particular repeat mode or =C-g=
