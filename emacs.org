...
#+begin_src emacs-lisp
  (use-package agent-shell
    ...
    :bind (("<f5>" . (lambda () (interactive)
                       (let ((default-directory "~/source/dsi-0"))
                         (agent-shell))))
           ("<f6>" . (lambda () (interactive)
                       (let ((default-directory "~/source/dsi-1"))
                         (agent-shell))))
    ...
    :init
    (defun agent-shell-anthropic-model-id-from-name (name)
    ...
    ;; *IMPROTANT* agent shell ignore the model specified in claude settings.json
    (agent-shell-anthropic-default-model-id (agent-shell-anthropic-model-id-from-name "opus")))
