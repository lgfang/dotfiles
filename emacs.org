...
If region is not active, copy or cut current line.

#+begin_src emacs-lisp
  (use-package emacs
    :bind
    (:map global-map ("M-w" . my/copy-region-or-line))

    :config
    (define-advice kill-region (:before (&rest _) region-or-line)
      "Cut region (if active) or current line."
      (interactive
       (if (use-region-p)
           (list (region-beginning) (region-end) t)
        (list (line-beginning-position) (line-beginning-position 2) nil))))

    ;; Not adding advice to `kill-ring-save' because it calls
    ;; `indicate-copied-region' when called interactively, which is inaccurate and
