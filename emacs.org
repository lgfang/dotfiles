...
#+begin_src emacs-lisp
  (use-package elfeed
    ...
    (elfeed-feeds
     '(;; Emacs
       ("https://www.reddit.com/r/emacs.rss" emacs forum reddit)
       ("https://emacs-china.org/latest.rss" emacs forum china)

       ;; Tech
       ("https://hnrss.org/frontpage" news tech hn)

       ;; Chinese & World News
       ("https://plink.anyfeeder.com/newscn/whxw" news china xinhua)
       ("https://plink.anyfeeder.com/zaobao/realtime/china" news china zaobao)
       ("https://plink.anyfeeder.com/zaobao/realtime/world" news world zaobao)

       ;; Social network hot
       ("https://rsshub.rss3.workers.dev/weibo/search/hot" news social china weibo)

       ;; Australia News
       ("https://www.abc.net.au/news/feed/51120/rss.xml" news au abc)
       ("https://www.theguardian.com/australia-news/rss" news au guardian)
       ("https://www.sbs.com.au/news/feed" news au sbs)
       ("https://www.smh.com.au/rss/feed.xml" news au smh)
       ("https://theconversation.com/au/articles.atom" news au conversation)
       ("https://www.news.com.au/national/rss" news au newscorp))
     ))
#+end_src
...
- Claude Code, see [[file:ai.org::*Claude][my Claude notes]] for details.
- Claude Code ACP
  #+begin_src bash
...
#+begin_src emacs-lisp
  (use-package agent-shell
    ...
    (defun my/start-agent-shell (&optional arg)
      ...
      (if arg
          (let* ((dirs '("~/source/project1/" "~/source/project2/" "~/source/project3/"))
                 (chosen-dir (completing-read "Select agent-shell directory: " dirs nil t))
                 (default-directory (expand-file-name chosen-dir)))
            (agent-shell))
        (agent-shell)))

    (defun my/anthropic-model-id-from-name (name)
      "Return a function that resolves model NAME to a model ID."
      (lambda ()
    ...
    ;; *IMPORTANT* agent shell ignore the model specified in claude settings.json
    (agent-shell-anthropic-default-model-id (my/anthropic-model-id-from-name "opus")))
#+end_src
