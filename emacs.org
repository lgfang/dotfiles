...
#+begin_src emacs-lisp
  (use-package org
    ...
    :config
    ;; Skip function for "worked on" agenda command
    ...
    (defun my/report-worked-on-tasks (days)
      ...
      (let* ((days-str (number-to-string days))
             (header (format "Worked on (last %s days)" days-str))
             (closed-match (format "CLOSED>=\"<-%sd>\"" days-str))
...
Prerequisites:
- Install the Agent Client Protocol (ACP):
  #+begin_src bash
    npm install -g @agentclientprotocol/claude-agent-acp
  #+end_src
- Install Claude Code (if you plan to use it), see [[file:ai.org::*Claude][my Claude notes]] for setup
  details.
- Install Opencode (if you plan to use it).
...
#+begin_src emacs-lisp
  (use-package agent-shell
    ...
    (defun my/anthropic-model-id-from-name (name)
      ...
      (lambda ()
        (when-let* ((models (and (boundp 'agent-shell--state)
          ...
          (when-let* ((matches (seq-filter (lambda (m)
                                             (string-match-p (downcase name)
                                                             (downcase (or (map-elt m :name) ""))))
                                           models))
                      (sorted (seq-sort (lambda (a b)
                                          (string-greaterp (or (map-elt a :model-id) "")
                                                           (or (map-elt b :model-id) "")))
                                        matches))
                      (latest-model (car sorted)))
            (map-elt latest-model :model-id)))))
    ...
    (agent-shell-anthropic-default-model-id (my/anthropic-model-id-from-name "recommended"))
    ;; Choose agent based on work/personal laptop.
    (agent-shell-preferred-agent-config
     (if my/work-machine-p 'claude-code 'opencode))
    ...
    ;; Configure MCPs based on work/personal laptop.
    (agent-shell-mcp-servers
     (if my/work-machine-p
         '(((name . "mdb-glean")
           ...
           ((name . "evergreen")
            (command . "uvx")
            (args . ("--from=git+https://github.com/evergreen-ci/evergreen-mcp-server"
